...
#: On-screen height, feet to crown, every character is normalised to.
BODY_H = 108


def snap_to_pixel_grid(cell, lo=48, hi=140, force=None):
    ...
    best, best_score = force, -1.0
    for n in [] if force else range(lo, hi + 1):
        step = w / n
        if step < 4:
            continue
        pure, blocks = 0.0, 0
...
def sheet_cells(path):
    ...
    w, h = img.size
    return [img.crop((cx * w // 2, cy * h // 2,
                      (cx + 1) * w // 2, (cy + 1) * h // 2))
            for cy in range(2) for cx in range(2)]


def load_pixel_parts(path, grid=None):
    """Parts from a chunky-pixel-art sheet, snapped onto their pixel grid by
    majority vote (no resampling): what the model drew is what ships."""
    snapped = [snap_to_pixel_grid(c, 24, 70, grid) for c in sheet_cells(path)]
    return [crop_opaque(s) for s in snapped], snapped[0].width


def body_height(parts):
    head, torso, _, leg = parts
    return head.height * 0.92 + torso.height + leg.height * 0.90


def export_parts(src, name, target=BODY_H):
    """Ship one character's head/torso/arm/leg as an imagetable and return
    the pivot offsets the runtime rig animates with.

    The model draws each sheet at whatever block size it likes, so the first
    snap finds the native grid and the second re-snaps on a grid scaled to
    put every character at the same on-screen height. Both passes are
    majority votes, so the art stays hard-edged either way.
    """
    path = os.path.join(RAW, src)
    parts, grid = load_pixel_parts(path)
    parts, _ = load_pixel_parts(
        path, max(8, round(grid * target / body_height(parts))))
    head, torso, arm, leg = parts
    ...
    prefix = "%sparts-table-" % name
    out = os.path.join(IMAGES, "%s%d-%d.png" % (prefix, cw, ch))
    ...
    lua = ["\t%s = {" % name, "\t\theight = %d," % round(body_h)]
    for key in order:
...
#: Raw part sheet -> character name used by the game and asset filenames.
CHARACTERS = (
    ("gen_male_parts_px.png", "hero"),
    ("gen_doc_parts_px.png", "doc"),
    ("gen_engineer_parts_px.png", "engineer"),
    ("gen_security_parts_px.png", "security"),
)
...
def main():
    ...
    for src, name in CHARACTERS:
        lua += export_parts(src, name)
    lua += ["}", ""]
    path = os.path.join(ROOT_SCRIPTS, "game", "rigs.lua")
